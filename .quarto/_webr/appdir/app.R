.libPaths(c("C:/Users/Usuario/AppData/Local/R/win-library/4.5", .libPaths()))
...
calcular_frecuencia_combinada_sintomas <- function(datos, vars_sintomas, var_evento = NULL ) {
  if (is.null(vars_sintomas) || length(vars_sintomas) == 0) {
    return(list(error = "No hay variables de síntomas definidas"))
  }
  
  # ==============================
  # Filtrar solo casos (evento == TRUE)
  # ==============================

  if (!is.null(var_evento)) {
    if (!var_evento %in% names(datos)) {
      return(list(error = sprintf("La variable de evento '%s' no existe en los datos", var_evento)))
    }
    col_evento <- convertir_a_logico(datos[[var_evento]])
    datos <- datos[!is.na(col_evento) & col_evento == TRUE, ]

    if (nrow(datos) == 0) {
      return(list(error = "No hay casos con evento == TRUE en los datos"))
    }
  }
...
calcular_frecuencia_simple_sintomas <- function(datos, vars_sintomas, var_evento = NULL) {
...
# =====================================================================
# FUNCIÓN AUXILIAR: GENERACIÓN ROBUSTA DE LA CURVA EPIDÉMICA
# =====================================================================
generar_grafico_curva_brote <- function(datos, var_inicio, config_yaml) {
  library(ggplot2)
  library(dplyr)

  # 1. Configuración y Validación Inicial
  unidad_tiempo_config <- if (!is.null(config_yaml$unidad_tiempo)) config_yaml$unidad_tiempo else NULL
  
  if (is.null(var_inicio) || !(var_inicio %in% names(datos))) {
    return(ggplot() + annotate("text", x = 0.5, y = 0.5, label = "Variable temporal no encontrada", size = 5, col = "red") + theme_void())
  }
  
  # Detectar tipo
  col_raw <- datos[[var_inicio]]
  es_posix <- inherits(col_raw, c("POSIXct", "POSIXt")) || 
              (!is.null(unidad_tiempo_config) && tolower(unidad_tiempo_config) %in% c("posix", "datetime", "fecha_hora"))
  
  es_fecha <- inherits(col_raw, "Date") || 
              (!is.null(unidad_tiempo_config) && tolower(unidad_tiempo_config) %in% c("fecha", "dia", "date"))
  
  es_hora <- is.numeric(col_raw) || 
             (!is.null(unidad_tiempo_config) && tolower(unidad_tiempo_config) %in% c("hora", "hour"))

  p <- NULL

  # --- BLOQUE 1: FECHAS SIMPLES (DATE) ---
  if (es_fecha) {
    datos_plot <- datos |> mutate(tiempo = as.Date(.data[[var_inicio]])) |> filter(!is.na(tiempo))
    
    if (nrow(datos_plot) == 0) return(ggplot() + annotate("text", x=0.5, y=0.5, label="No hay datos válidos", size=5) + theme_void())
    
    datos_count <- datos_plot |> group_by(tiempo) |> summarise(n_casos = n(), .groups = "drop")
  ...
  # --- BLOQUE 2: HORAS NUMÉRICAS ---
    ...
    datos_count <- datos_plot |> group_by(tiempo) |> summarise(n_casos = n(), .groups = "drop")
    rango_horas <- seq(min(datos_count$tiempo), max(datos_count$tiempo), by = 1)
  ...
  # --- BLOQUE 3: POSIX (FECHA + HORA) - OPTIMIZADO ---
    ...
    tiempo_vec <- if (inherits(col_raw, c("POSIXct", "POSIXt"))) {
      col_raw
    } else {
      as.POSIXct(sub("T", " ", sub("Z$", "", as.character(col_raw))), tz = "UTC")
    }
    
    df_temp <- data.frame(tiempo = tiempo_vec) |> filter(!is.na(tiempo))
    
    if (nrow(df_temp) == 0) return(ggplot() + annotate("text", x=0.5, y=0.5, label="No hay datos POSIX válidos", size=5) + theme_void())
    ...
    # Rango completo para evitar huecos en el gráfico
    rango_completo <- seq(from = min(df_temp$tiempo_trunc), to = max(df_temp$tiempo_trunc), by = by_str)
    
    datos_count_completo <- data.frame(tiempo = rango_completo) |>
      left_join(datos_count, by = c("tiempo" = "tiempo_trunc")) |>
      mutate(n_casos = ifelse(is.na(n), 0, n))
...
crear_forest_plot <- function(resultados, tipo = "OR") {
  ...
  caption_text <- "Líneas verticales representan intervalos de confianza al 95%"
  ...
  if (is.null(resultados_tasas) || length(resultados_tasas) == 0) {
    return(div(
      ...
      tags$ul(
        ...
        tags$li("Verifique que las columnas de los datos coincidan con las especificadas en el YAML")
      ),
...
control_calidad_fechas <- function(datos, vars_solicitud) {
  ...
  # ---- Validar unidad ----
  if (!unidad %in% c("fecha", "fecha_hora", "hora")) {
    res$ok     <- FALSE
    res$errores <- c(res$errores,
                     "unidad_tiempo debe ser 'fecha', 'fecha_hora' u 'hora'")
    return(res)
  }

  # ---- Existencia de variable_tiempo_inicio ----
  ...
  # ---- Resolver fecha_ingesta: columna vs. constante ----
  ingesta_es_columna <- !is.null(var_ingesta) && is.character(var_ingesta) &&
                          var_ingesta %in% names(datos)
  ingesta_es_constante <- !is.null(var_ingesta) && !ingesta_es_columna

  if (!is.null(var_ingesta) && ingesta_es_columna) {
    y <- datos[[var_ingesta]]
  } else if (ingesta_es_constante) {
    y <- var_ingesta          # escalar; se expande al comparar
  } else {
    y <- NULL
  }
  ...
  if (unidad == "fecha") {
    ...
    if (!inherits(x, "Date")) {
      ...
      if (inherits(x2, "try-error")) {
        res$ok <- FALSE
        res$errores <- c(res$errores,
                         "variable_tiempo_inicio no es convertible a Date")
        return(res)
      }
      x <- x2
      res$advertencias <- c(res$advertencias,
                             "variable_tiempo_inicio convertida a Date")
    }
    ...
    if (!is.null(y)) {
      if (!inherits(y, "Date")) {
        ...
        if (inherits(y2, "try-error")) {
          res$ok <- FALSE
          res$errores <- c(res$errores, "fecha_ingesta no es convertible a Date")
          return(res)
        }
        y <- y2
  ...
  if (unidad == "fecha_hora") {

    # variable_tiempo_inicio debe ser POSIXct o cadena convertible
    if (!inherits(x, "POSIXct")) {
      x2 <- try(as.POSIXct(x, tz =  "UTC"), silent = TRUE)
      if (inherits(x2, "try-error")) {
        res$ok <- FALSE
        res$errores <- c(res$errores,
                         "variable_tiempo_inicio no es convertible a POSIXct")
        return(res)
      }
      x <- x2
      res$advertencias <- c(res$advertencias,
                             "variable_tiempo_inicio convertida a POSIXct")
    }

    if (!is.null(y)) {
      if (!inherits(y, "POSIXct")) {
        y2 <- try(as.POSIXct(y, tz ="UTC"), silent = TRUE)   # acepta "YYYY-MM-DD HH:MM"
        if (inherits(y2, "try-error")) {
          res$ok <- FALSE
          res$errores <- c(res$errores,
            "fecha_ingesta no es convertible a POSIXct (use 'YYYY-MM-DD HH:MM')")
          return(res)
        }
        y <- y2
        res$advertencias <- c(res$advertencias,
                               "fecha_ingesta convertida a POSIXct")
      }
    }
  }
  ...
  if (unidad == "hora") {
    ...
    # variable_tiempo_inicio: numérico
    if (!is.numeric(x)) {
      x2 <- suppressWarnings(as.numeric(x))
      if (all(is.na(x2))) {
        res$ok <- FALSE
        res$errores <- c(res$errores,
                         "variable_tiempo_inicio no es convertible a numérico")
        return(res)
      }
      x <- x2
      res$advertencias <- c(res$advertencias,
                             "variable_tiempo_inicio convertida a numérico")
    }

    # fecha_ingesta constante: puede llegar como character "8" o "8.5"
    if (!is.null(y)) {
      if (is.character(y)) {
        ...
        if (is.na(y2)) {
          res$ok <- FALSE
          res$errores <- c(res$errores,
            "fecha_ingesta (hora) no es convertible a numérico")
          return(res)
        }
        y <- y2
        res$advertencias <- c(res$advertencias,
          "fecha_ingesta llegó como character; convertida a numérico. ",
          "Considere definirla sin comillas en el YAML.")
      } else if (!is.numeric(y)) {
        res$ok <- FALSE
        res$errores <- c(res$errores,
                         "fecha_ingesta debe ser numérica para unidad 'hora'")
        return(res)
      }
    }
  }
  ...
  if (!is.null(y)) {
    ...
    # Expandir constante escalar al tamaño de x
    if (length(y) == 1) y <- rep(y, length(x))

    incub <- if (unidad %in% c("fecha", "fecha_hora")) {
      as.numeric(difftime(x, y,
                          units = if (unidad == "hora") "hours" else
                                  if (unidad == "fecha_hora") "hours" else "days"))
    } else {
    ...
    if (length(incub) == 0) {
      res$ok <- FALSE
      res$errores <- c(res$errores,
                       "No hay pares válidos para calcular incubación")
      return(res)
    }

    if (any(incub < 0)) {
      res$advertencias <- c(res$advertencias,
                             "Existen incubaciones negativas (posible error en fechas)")
    }

    umbral_horas <- switch(unidad,
      hora       = 720,
      fecha_hora = 720,
      fecha      = 30   # días
    )
    if (any(incub > umbral_horas)) {
      res$advertencias <- c(res$advertencias,
        paste0("Incubaciones >", umbral_horas,
               if (unidad == "fecha") " días" else " horas", " detectadas"))
    }
...
calcular_incubacion <- function(datos, vars_solicitud) {
  ...
  if (is.null(vars_solicitud$variable_tiempo_inicio) ||
    ...
    return(list(error =
      "No se definió variable_tiempo_inicio o unidad_tiempo en YAML"))
  }
  ...
  if (!var_inicio %in% names(datos)) {
    return(list(error =
      "La variable de inicio de síntomas no existe en los datos"))
  }

  inicio <- datos[[var_inicio]]

  # ---- Resolver fecha_ingesta ----
  if (is.null(vars_solicitud$fecha_ingesta)) {
    return(list(error = "No se definió fecha_ingesta en YAML"))
  }

  fi <- vars_solicitud$fecha_ingesta

  if (is.character(fi) && fi %in% names(datos)) {
    # Es el nombre de una columna
    ingesta <- datos[[fi]]
  } else {
    # Es una constante: escalar que se repetirá
    ingesta <- fi
  }

  # ---- Conversión y cálculo según unidad ----

  if (unidad == "fecha_hora") {

    # inicio → POSIXct
    if (!inherits(inicio, "POSIXct")) {
      inicio <- tryCatch(
        as.POSIXct(inicio),
        error = function(e) stop(
          "variable_tiempo_inicio no es convertible a POSIXct: ", e$message)
      )
    }

    # ingesta → POSIXct (acepta "YYYY-MM-DD HH:MM" o vector de columna)
    if (!inherits(ingesta, "POSIXct")) {
      tz_uso  <- tryCatch(attr(inicio, "tzone") %||% "", error = function(e) "")
      ingesta <- tryCatch(
        as.POSIXct(ingesta, tz = "UTC"),
        error = function(e) stop(
          "fecha_ingesta no es convertible a POSIXct: ", e$message)
      )
    }

    if (length(ingesta) == 1) ingesta <- rep(ingesta, nrow(datos))
    ...
    # inicio numérico
    if (!is.numeric(inicio)) {
      inicio <- suppressWarnings(as.numeric(inicio))
    }

    # ingesta: constante numérica o columna numérica
    if (is.character(ingesta)) {
      ingesta <- suppressWarnings(as.numeric(ingesta))
      if (is.na(ingesta)) {
        return(list(error =
          paste0("fecha_ingesta '", fi,
                 "' no es convertible a numérico. ",
                 "Defínala sin comillas en el YAML.")))
      }
    }
    if (!is.numeric(ingesta)) {
      ingesta <- as.numeric(ingesta)
    }

    if (length(ingesta) == 1) ingesta <- rep(ingesta, length(inicio))

    incubacion   <- inicio - ingesta
    unidad_salida <- "horas"

  } else {  # "fecha"

    inicio  <- as.Date(inicio)
    ingesta <- as.Date(ingesta)   # funciona con columna o constante "YYYY-MM-DD"
  ...
  if (length(incubacion) < 3) {
    return(list(error =
      "No hay suficientes datos válidos para calcular incubación"))
  }
...
clasificar_incubacion <- function(mediana, unidad) {
  ...
  # Normalizar siempre a horas para comparar
  mediana_h <- if (unidad == "días") mediana * 24 else mediana

  if (mediana_h <= 6) {
    "Mediana menor o igual a 6 horas"
  } else if (mediana_h <= 16) {
    "Mediana menor o igual a 16 horas"
  } else if (mediana_h <= 48) {
    "Mediana menor o igual a 48 horas"
  } else {
    "Mediana mayor a 48 horas"
  }
}


# -----------------------------------------------------------------------------
# HELPER: operador %||% (null-coalescing) si no está disponible
# -----------------------------------------------------------------------------
...
convertir_incubacion_a_html <- function(res) {
  ...
  if (is.null(res)) {
    return("<p class='alert alert-warning'>No se pudo calcular la incubación</p>")
  }
  ...
  filas_html <- paste0(
    "<tr><td>", names(tabla), "</td><td>",
    round(unlist(tabla), 2),
    "</td></tr>",
    collapse = "\n"
  )

  html <- sprintf("
    <h3>Periodo de incubación</h3>
    ...
    <div class='alert alert-info'>
      <strong>Interpretación epidemiológica orientativa:</strong><br>
      %s
    </div>

    <div class='alert alert-secondary'>
      <small>
        El periodo de incubación se estimó como diferencia entre inicio de síntomas e ingesta.
        Resultados orientativos sujetos a la calidad de los datos.
      </small>
    </div>
  ",
  filas_html,
...
datos_procesados <- reactive({
    req(estado$datos, estado$config) # Requiere que el CSV y el YAML estén listos
    
    df <- estado$datos
    vars <- estado$config
    var_tiempo <- vars$variable_tiempo_inicio
    
    # Si la variable de tiempo existe en el dataset, la transformamos de inmediato
    if (!is.null(var_tiempo) && var_tiempo %in% names(df)) {
      
      # Intentamos parsear el formato estandarizado ISO 8601 (2001-09-22T22:00:00Z)
      tiempo_convertido <- as.POSIXct(df[[var_tiempo]], format = "%Y-%m-%dT%H:%M:%OSZ", tz = "UTC")
      
      # Fallback 1: Si da todo NA, probamos sin la Z estricta
      if (all(is.na(tiempo_convertido))) {
        tiempo_convertido <- as.POSIXct(df[[var_tiempo]], format = "%Y-%m-%dT%H:%M:%S")
      }
      
      # Fallback 2: Dejamos que R intente adivinar automáticamente si sigue dando NA
      if (all(is.na(tiempo_convertido))) {
        tiempo_convertido <- as.POSIXct(df[[var_tiempo]])
      }
      
      # Reemplazamos la columna por el objeto cronológico real (POSIXct)
      df[[var_tiempo]] <- tiempo_convertido
    }
...
output$resultado <- renderUI({
  ...
  output$tabla_tasas <- renderUI({
    ...
    return(render_tabla_tasas(resultados_tasas))
  })
 ...
 # NUEVO: Output para riesgo relativo
  output$tabla_rr <- renderUI({
    ...
    return(render_tabla_rr(resultados_rr))
  })
  ...
  output$tabla_or <- renderUI({
    ...
    return(render_tabla_or(resultados_or))
  })
...
# NUEVO: Forest plot para RR
  ...
  # 1. Traer la fuente única de verdad procesada
  info <- informe()
  if (!is.null(info$error)) return(NULL)
  
  datos_plot     <- info$datos
  vars_solicitud <- info$vars  # Esto es exactamente estado$solicitud$variables
  
  var_inicio <- info$vars$variable_tiempo_inicio
  
  if (is.null(var_inicio) || var_inicio == "") {
    var_inicio <- "hora_presentacion" # Nombre real que confirmaste existe
  }

  # 2. Extraer parámetros clave del YAML
 # var_inicio    <- vars_solicitud$variable_tiempo_inicio
  unidad_config <- vars_solicitud$unidad_tiempo
  
  # 3. Validación de existencia de la columna en el set de datos
  if (is.null(var_inicio) || !(var_inicio %in% names(datos_plot))) {
    #columnas_disponibles <- paste(head(names(datos_plot), 5), collapse = ", ")
    return(ggplot2::ggplot() + 
             ggplot2::annotate("text", x = 0.5, y = 0.5, 
                      label = paste0("Error: La columna '", var_inicio, "' no se encuentra en estado$datos.\n",
                                     "Columnas encontradas: ", paste(names(estado$datos), collapse=", ")), 
                      size = 3, color = "red") + 
             ggplot2::theme_void())
  }
  
  # 4. Forzar el formato POSIXct si el YAML lo indica explícitamente
  # Esto soluciona el problema de los textos con formato ISO 8601 (con T y Z)
  if (!is.null(unidad_config) && tolower(unidad_config) %in% c("posix", "datetime", "fecha_hora")) {
    col_raw <- datos_plot[[var_inicio]]
    if (!inherits(col_raw, c("POSIXct", "POSIXt"))) {
      # Limpieza de caracteres de zona horaria ISO comunes antes de parsear
      col_str <- sub("Z$", "", as.character(col_raw))
      col_str <- sub("T", " ", col_str)
      datos_plot[[var_inicio]] <- as.POSIXct(col_str, format = "%Y-%m-%d %H:%M:%S", tz = "UTC")
    }
  }
  
  # 5. Generar la curva epidémica con la función analítica robusta
  p <- generar_grafico_curva_brote(
    datos       = datos_plot,
    var_inicio  = var_inicio,
    config_yaml = vars_solicitud
  )
  
  # 6. Persistir el gráfico en el estado para que la descarga HTML/JS funcione
  estado$plot_curva <- p
  
  # 7. Renderizar en la interfaz de Shiny
  p
})
...
output$tabla_freq_simple <- renderUI({
    ...
    if (is.null(vars$sintomas)) {
      return(div(class = "alert alert-warning", "No hay variables de síntomas definidas en el YAML"))
    }
    
    tabla_freq <- calcular_frecuencia_simple_sintomas(datos = info$datos, vars_sintomas = vars$sintomas, vars$evento)
    estado$freq_simple <- tabla_freq
    
    if (!is.null(tabla_freq$error)) return(div(class = "alert alert-warning", tabla_freq$error))
    ...
    tags$div(
      ...
      tags$div(
        ...
        tags$small(tags$strong("Nota:"), " Muestra la frecuencia individual ordenada.")
      )
    )
  })
...
observeEvent(input$btn_descargar_js, {
  ...
  ev <- estado$solicitud$definicion_problema$evento
  ...
  curva_img  <- get_b64(estado$plot_curva)
  ...
  # Añadir desc univariada
  if(!is.null(tabla_desc_html)) {  
  ...
  if(!is.null(forest_rr_img)) {
    contenido <- c(contenido, list(
      ...
      img(src = forest_rr_img, style="width:70%; border:1px solid #eee;")
    ))
  }
  ...
  if(!is.null(forest_or_img)) {
    contenido <- c(contenido, list(
      h2("Forest Plot"),
      img(src = forest_or_img, style="width:70%; border:1px solid #eee;")
    ))
  }
  ...
  if(!is.null(curva_img)) {
    contenido <- c(contenido, list(
      h2("Curva Epidémica"),
      img(src = curva_img, style="width:70%; border:1px solid #eee;")
    ))
  }
